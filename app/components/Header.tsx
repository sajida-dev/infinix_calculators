    ...
    <header className="sticky top-4 z-50 w-full px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto mb-6">
      ...
      >
            ...
            <div className="absolute left-0 mt-1 w-56 rounded-xl border border-slate-200 dark:border-[#4D5156] bg-white dark:bg-[#22242A] p-3 shadow-lg hidden group-hover:block z-50">
              <ul className="space-y-1">
                {categoriesList.map((cat) => (
                  <li key={cat.id}>
                    <Link href={`/calculators#${cat.id}`} className="block rounded-lg px-3 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/60 hover:text-primary transition-colors">
                      {cat.title}
                    </Link>
                  </li>
                ))}
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Mobile Menu Button */}
          <button
            className="md:hidden p-2 rounded-xl hover:bg-slate-100/80 dark:hover:bg-slate-800/80 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white transition"
      ...
      {open && (
        <div className="md:hidden rounded-2xl border border-slate-200/80 dark:border-[#4D5156] bg-white/95 dark:bg-[#22242A]/95 backdrop-blur-md p-4 mt-2 shadow-lg flex flex-col gap-2 transition-all duration-300">
          <Link
            href="/"
            className="rounded-lg px-3 py-2 text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/60 hover:text-primary transition-colors"
          ...
          <div className="pl-4 flex flex-col gap-1 border-l border-slate-100 dark:border-slate-800 ml-3">
            {categoriesList.map((cat) => (
              <Link
                key={cat.id}
